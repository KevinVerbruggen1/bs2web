  ...
  <form action="Controller?command=Edit&userId=${userId}&editUser=true" method="post" novalidate="novalidate">
    ...
    <p>
      ...
      <select id="role" name="role">
        <option value="EMPLOYEE" ${rolePreviousValue eq 'employee'?'selected':''}>employee</option>
        <option value="DIRECTOR" ${rolePreviousValue eq 'admin'?'selected':''}>admin</option>
        <option value="TEAMLEADER" ${rolePreviousValue eq 'teamleader'?'selected':''}>teamleader</option>
      </select>
    </p>
    <p>
